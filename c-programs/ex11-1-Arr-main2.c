 ...
 * The menu driven program to test Arr (using an array of char)
 */
#include <stdlib.h>
#include <stdio.h>
...
void help() {
    printf("This is a dynamic array of char arrays values. You can create up to 5 arrays\n");
...
void menu() {
  ...
  while(1) {
	...
	case 'G':
		...
		/* pc = *((char*)p); */
		printf("%d-th array[%d] = %s\n", i, number, (char*)p);
		break;
	case 's':
	case 'S':
		printf("Enter array # (between 0 and 4), the element index, the value to be set (a word): ");
		if(scanf("%d%d%s", &i, &number, pc) != 3 || i < 0 || i > 4) {
			fprintf(stderr, "invalid values\n");
			SKIP
			break;
		}
	...
	case 'e':
	case 'E':
		printf("Enter enumeration # (between 0 and 4) and the array (to be enumerated) #: ");
