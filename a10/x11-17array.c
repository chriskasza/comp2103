...
/*
 * Function:  killEnd
 * Purpose:   to free the space occupied by the last pos specified bytes
 *             of the array
 * Inputs:    this, pos
 * Returns:   0 if the array is not initialized or pos is incorrect, or
 *             the memory reallocation failed; 1 otherwise
 * Modifies:  this array
 * Error checking:   checks to ensure pos is correct, and memory
 *            reallocation is successful
 */
int killEnd( Abstract_Arr this, int pos ){

	if( pos < 0 || pos >= this->size_)
		return 0;

	this->size_ = this->size_ - pos;
	this->block_ = realloc (this->block_, this->elSize_ * this->size_);
...
int hasMoreElements_Enumeration(Abstract_Enumeration e) {
  /*printf("line size is %d; line init'd? %d\n", e->arr->size_, e->arr->init_);*/
	assert(e->arr->init_);
