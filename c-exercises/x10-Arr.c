...
/*
 * Function:  shrink_
 * Purpose:  private function to shrink the memory to the specified size
 * Inputs:   size
 * Returns:  0 on failure; 1 otherwise
 * Modifies:  the internal array
 * Error checking:  makes sure the incoming size is greater than 0 and less than
 *		current array size; ensures that the internal state is intialized
 */
static int shrink_( int size ){
   Element_Arr *new;
   int i;
...
/*
 * Function:  shrink_Arr
 * Purpose:  to shrink the memory to the specified size
 * Inputs:   size
 * Returns:  0 on failure; 1 otherwise
 * Modifies:  the internal array
 * Error checking:  makes sure the incoming size is greater than 0 and less than
 *					current array size; ensures that the internal state is intialized
 */
int shrink_Arr(int size){
	
	if(!init_ || size < 0 || size > size_)
		return 0;
